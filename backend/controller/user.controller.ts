...
import {
  SignUpData,
  LoginData
} from '../Request/userData';

dotenv.config();

const signUp = async (req: Request, res: Response) => {
  try {
    const { email, password, firstName, lastName, address } = req.body as SignUpData;
    ...
    return void res.status(201).json({ message: 'User created successfully', userId: newUser.id });
  } catch (error) {
    return void res.status(500).json({ message: 'Error to register User', error });
  }
};
...
const getProfile = async (req: Request, res: Response) => {
  try {
    ...
    if (!userId) {
      return void res.status(401).json({ message: 'Unable to show user profile' });
    }
    ...
    return void res.status(401).json({ message: 'Error to obtain user profile', error });
  }
};
